  ...
  const countStage = async (stage:string, fromDate?:string, toDate?:string) => {
    ...
    if (stage !== 'All Leads') {
      if (stage === 'Duplicate Leads') q = q.eq('crm_status', 'Duplicate Leads');
        ...
        // New pipeline (crm_status) + legacy fallback (lead_status='Lost')
        q = q.or(`crm_status.eq.Not Interested,and(crm_status.is.null,lead_status.eq.Lost)`);
      } else {
        q = q.or(`crm_status.eq.${stage},and(crm_status.is.null,lead_status.eq.${stage})`);
      }
    }
    ...
    if (error) {
      ...
      if (/team/i.test(error.message||'')) {
        ...
        if (stage !== 'All Leads') {
          if (stage === 'Not Interested') retry = retry.or(`crm_status.eq.Not Interested,and(crm_status.is.null,lead_status.eq.Lost)`);
          else if (stage !== 'Duplicate Leads') retry = retry.or(`crm_status.eq.${stage},and(crm_status.is.null,lead_status.eq.${stage})`);
          else retry = retry.eq('crm_status', 'Duplicate Leads');
        }
