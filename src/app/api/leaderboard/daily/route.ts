...
export async function GET(request: Request) {
  ...
  const [usersRes, logsRes] = await Promise.all([
    ...
    db.from('activity_log').select('user_id, action_type').gte('created_at', start).lte('created_at', end),
  ]);

  const users = usersRes.data||[];
  // Also count call_logs and follow_ups for daily actions
  const { data: calls } = await db.from('call_logs').select('user_id').gte('created_at', start).lte('created_at', end);
  const { data: follows } = await db.from('follow_ups').select('created_by').gte('created_at', start).lte('created_at', end);

  const countBy = (arr:any[], key:string, uid:string) => arr.filter((x:any)=> x[key]===uid).length;

  const ranked = users.map((u:any)=>{
    const actions = countBy(logsRes.data||[], 'user_id', u.id) + countBy(calls||[], 'user_id', u.id) + countBy(follows||[], 'created_by', u.id);
