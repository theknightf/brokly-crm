  ...
  if (error || !data) {
    return (
      <div className="bg-card border border-border rounded-2xl py-16 text-center">
        ...
        <button onClick={load} className="btn-primary h-9 px-4 text-sm flex items-center gap-1.5 mx-auto">
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Attendance %</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Avg Working Hours</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Activity</th>
              ...
              {data.performance.slice(0, 12).map((p) => (
                <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <Link href={`/admin/employees/${p.id}`} className="flex items-center gap-3 group">
                      <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                        <span className="text-xs font-bold text-primary">
                          {p.name.split(' ').map((x) => x[0]).join('').toUpperCase().slice(0, 2)}
                        </span>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-foreground group-hover:text-primary truncate transition-colors">{p.name}</p>
                  ...
                  <td className="px-5 py-3 text-right">
                    <Link href={`/admin/employees/${p.id}`} className="btn-ghost p-1.5 inline-flex" title="View employee report">
        ...
        {data.performance.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-10">No employee data available yet.</p>
        )}
      </div>
              ...
              <div className="mt-4 bg-teal-soft rounded-xl p-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {data.bestEmployee.name} · {data.bestEmployee.attendanceRate}% attendance · {data.bestEmployee.avgHours}h
                  </p>
                </div>
              ...
              <li key={ev.id} className="ml-4">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {new Date(ev.createdAt).toLocaleString([], { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })}
                  </span>
                </div>
