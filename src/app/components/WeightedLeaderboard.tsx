  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      : <div className="divide-y divide-border">
          {users.map(u => (
            <div key={u.user_id} className={`p-3 flex items-center gap-3 ${u.rank<=3 ? 'bg-gradient-to-r from-violet-50/50 to-transparent' : ''}`}>
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-foreground truncate flex items-center gap-1">{u.full_name || u.email} <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${u.rank===1 ? 'bg-amber-100 text-amber-700' : 'bg-muted text-muted-foreground'}`}>{u.role}</span></p>
