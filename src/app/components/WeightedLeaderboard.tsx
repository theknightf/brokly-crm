...
function RankIcon({ rank }: { rank: number }) {
  if (rank === 1) return <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center border border-lime-500/40 shadow-[0_0_10px_rgba(163,230,53,0.1)]"><Crown size={14} className="text-white"/></div>;
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      : <div className="divide-y divide-border">
          {users.map(u => (
            <a key={u.user_id} href={`/admin/employees/${u.user_id}/360`} className={`p-3 flex items-center gap-3 hover:bg-lime-50 dark:hover:bg-lime-500/10 transition-colors cursor-pointer ${u.rank<=3 ? 'bg-gradient-to-r from-lime-50/50 to-transparent dark:from-lime-500/5 dark:to-transparent' : ''}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex gap-3 mt-1">
                  ...
                  <ScoreBar label="Dress 30%" value={u.scores.dressScore} icon={<Shirt size={10} className="text-lime-400"/>} color="bg-lime-400"/>
                </div>
              </div>
