...
export default function UnitsScreen() {
  ...
  const filtered = units.filter((u) => {
    ...
    if (q) {
      const hay = [u.name, u.unitType, u.projectName, u.paymentPlan].filter(Boolean).join(' ').toLowerCase();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button onClick={() => { setEditing(null); setFormOpen(true); }} className="btn-primary flex items-center gap-1.5 text-sm">
          <Plus size={15} />
          Add Unit
        </button>
      </div>
      ...
      <div className="sm:hidden space-y-3">
        {loading ? (
          ...
          <EmptyState icon={<Building2 size={22} className="text-muted-foreground" />} title="No units" description="Add units to your inventory to recommend them in the calculator." />
        ) : (
          filtered.map((u) => (
            <div key={u.id} className="bg-card border border-border rounded-2xl p-3.5 shadow-sm space-y-2">
              <div className="flex items-start gap-3">
                {coverUrls[u.id] ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={coverUrls[u.id]} alt={u.name} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {[u.unitType, u.area ? `${u.area} m²` : '', u.floor ? `Floor ${u.floor}` : ''].filter(Boolean).join(' · ') || '—'}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => { setEditing(u); setFormOpen(true); }} className="w-9 h-9 rounded-xl hover:bg-secondary text-muted-foreground flex items-center justify-center" aria-label="Edit">
                    <Pencil size={15} />
                  </button>
                  <button onClick={() => setDeleting(u)} className="w-9 h-9 rounded-xl hover:bg-red-50 text-muted-foreground hover:text-red-500 flex items-center justify-center" aria-label="Delete">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
      ...
      {/* Desktop table */}
      <div className="hidden sm:block card-base !p-0 overflow-hidden">
                ...
                <tr key={u.id} className="hover:bg-muted/30 transition-colors">
                  <td className="table-td">
                    <div className="flex items-center gap-2.5">
                      {coverUrls[u.id] ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={coverUrls[u.id]} alt={u.name} className="w-9 h-9 rounded-lg object-cover flex-shrink-0" />
                      ) : (
                  ...
                  <td className="table-td text-right tabular-nums">{u.area ? `${u.area} m²` : '—'}</td>
                  <td className="table-td text-right font-semibold tabular-nums">EGP {fmt(Number(u.price || 0))}</td>
                  <td className="table-td text-muted-foreground max-w-[180px] truncate">{u.paymentPlan || '—'}</td>
                  <td className="table-td">
                    <select
                      value={u.status}
                      onChange={(e) => changeStatus(u, e.target.value)}
      ...
      <Modal open={!!deleting} onClose={() => setDeleting(null)} title="Delete unit" size="sm">
        <div className="p-6 space-y-4">
          <p className="text-sm text-foreground">
            Delete unit <span className="font-semibold">{deleting?.name}</span>? This removes it from
            projects and recommended-unit lists.
          </p>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" min={0} value={form.area || ''} onChange={(e) => setForm((f: any) => ({ ...f, area: num(e.target.value) }))} className="input-base" />
          </div>
          <div>
            <label className="label-base">Floor</label>
            <input type="number" value={form.floor || ''} onChange={(e) => setForm((f: any) => ({ ...f, floor: num(e.target.value) }))} className="input-base" />
          </div>
          <div>
            <label className="label-base">Price (EGP)</label>
            <input type="number" min={0} value={form.price || ''} onChange={(e) => setForm((f: any) => ({ ...f, price: num(e.target.value) }))} className="input-base" />
          </div>
        </div>
        <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" min={0} max={100} value={form.downPaymentPct || ''} onChange={(e) => setForm((f: any) => ({ ...f, downPaymentPct: num(e.target.value) }))} className="input-base" />
          </div>
          <div>
            <label className="label-base">Years</label>
            <input type="number" min={0} value={form.installmentYears || ''} onChange={(e) => setForm((f: any) => ({ ...f, installmentYears: num(e.target.value) }))} className="input-base" />
          </div>
          <div>
            <label className="label-base">Instalments / yr</label>
            <select value={String(form.installmentFrequency || 12)} onChange={(e) => setForm((f: any) => ({ ...f, installmentFrequency: num(e.target.value, 12) }))} className="input-base appearance-none">
              {[1, 2, 4, 12].map((n) => (
                <option key={n} value={n}>{n === 12 ? 'Monthly' : n === 4 ? 'Quarterly' : n === 2 ? 'Semi-annual' : 'Annual'}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={onPickCover} />
            <button
              type="button"
            ...
            >
              ...
              {uploading ? 'Uploading…' : initial?.id ? 'Upload cover photo' : 'Save unit first, then upload'}
            </button>
