...
import DeductionAlertProvider from './DeductionAlertProvider';
...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className={`pwa-shell flex min-h-screen bg-background overflow-hidden supports-[height:100dvh]:h-[100dvh] ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      {/* Mobile overlay */}
      {mobileSidebarOpen && (
        <div
      ...
      <QuickActions />
      {/* Instant deduction dispatcher (spec 4.B) */}
      <DeductionAlertProvider />
    </div>
  );
}
